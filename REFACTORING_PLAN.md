...
## 🚧 В РАБОТЕ (Этап 2: Миграция компонентов)

### Приоритет 1: Критичные компоненты
- [ ] Заменить AuthContext.tsx на AuthContext.new.tsx
  - Переименовать старый в AuthContext.old.tsx
  - Переименовать новый в AuthContext.tsx
  - Проверить все импорты
  
- [ ] Обновить App.tsx
  - Удалить AuthProvider (уже есть в main.tsx через Redux)
  - Проверить ProtectedRoute
  
- [ ] Обновить src/lib/api.ts
  - Удалить старый api объект
  - Создать api на базе apiClient из src/api/apiClient.ts
  - Мигрировать все методы

### Приоритет 2: Страницы с тяжелой логикой
- [ ] Dashboard.tsx
  - Заменить fetch на apiClient
  - Использовать useAppSelector для userData
  - Вынести логику фильтрации в селектор
  
- [ ] Objects.tsx
  - Использовать objectsSlice вместо userData.objects
  - Использовать createObject thunk вместо api.createItem
  
- [ ] WorkDetail.tsx
  - Использовать worksSlice
  - Рефакторинг табов (вынести в отдельные компоненты)

- [ ] InspectionDetail.tsx
  - Использовать inspectionsSlice
  - Рефакторинг логики дефектов

### Приоритет 3: Формы создания
- [ ] Создать универсальный EntityForm
  - Конфигурация для object/work/inspection
  - Валидация через zod
  - Единая логика отправки через Redux

- [ ] Заменить все формы создания на EntityForm:
  - CreateObject → EntityForm config
  - CreateWork → EntityForm config
  - CreateInspectionSimple → EntityForm config
